...
Halaman ini berisi daftar lengkap dokumen yang harus disiapkan untuk pendaftaran PPDGS USU.

```mermaid
flowchart TB
    subgraph Tab Data Diri
        A[Foto Terbaru]
    end

    subgraph Tab Pendidikan Kedokteran
        B[Ijazah S1 Kedokteran]
        C[Ijazah Profesi Dokter]
    end

    subgraph Tab Pendidikan Non Formal
        D[Sertifikat/Lisensi Non Formal]
    end

    subgraph Tab Karya Tulis
        E[Cover Karya Tulis]
        F[Halaman Pertama Karya Tulis]
    end

    subgraph Tab Prestasi
        G[Sertifikat Prestasi]
    end

    subgraph Menu Pembayaran
        H[Bukti Pembayaran]
    end
```
