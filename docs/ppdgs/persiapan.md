...
```mermaid
flowchart LR
    A[Mulai Persiapan] --> B{Dokumen Siap?}
    B -->|Ya| C[Data Diri Lengkap]
    B -->|Tidak| D[Siapkan Dokumen]
    D --> B
    C --> E[Koneksi Stabil]
    E --> F[Siap Mendaftar]
```
...
Setelah semua persiapan selesai, lanjut ke [Registrasi Akun](/ppdgs/registrasi-akun).
